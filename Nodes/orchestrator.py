...
from Schema.task_schema import OrchestratorPlan
    ...
    def orchestrator_node(state: SupplyChainState, config: RunnableConfig):
        ...
        try:
            ...
            if plan.is_workflow_complete:
                logger.info(f"✅ Workflow complete. Direct final answer generated by Orchestrator.")
                return {
                ...
                logger.info(f"👷 Orchestrator created {len(plan.tasks)} task(s) for worker execution:")
                for t in plan.tasks:
                    logger.info(f"   🔹 [{t.task_id}] Worker: '{t.assigned_worker}' | Type: '{t.tool_type}' | Task: '{t.description}'")
                return {
                    "is_workflow_complete": False, 
