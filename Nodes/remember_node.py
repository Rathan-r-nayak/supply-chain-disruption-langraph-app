...
class MemoryFact(BaseModel):
    # 🌟 FIX 1: Updated description to fit logistics, removing the old "finances" mention
    fact: str = Field(description="A distinct, standalone fact about the user's personal identity or long-term working preferences.")
...
def remember_node(state: SupplyChainState, config: RunnableConfig, store: BaseStore):
    ...
    if not messages:
        logger.info("ℹ️ No messages in state. Skipping LTM fact extraction.")
        return {}
    ...
    logger.info("🧠 Running LLM fact extraction on recent conversation turns...")
    ...
    # 🌟 FIX 2: Added explicit negative constraints to the prompt
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are an advanced Long-Term Memory (LTM) extraction agent for a logistics system. 
    Extract ONLY permanent, durable user facts, working preferences, or explicit rules stated by the user.
    
    🛑 DO NOT EXTRACT THE FOLLOWING (Transient/Operational Data):
    - Active trip details (trip IDs, origins, destinations, checkpoints)
    - Vehicle assignments, types, or registration numbers
    - Real-time operational statuses (e.g., IN_TRANSIT, current location)
    - Temporary notifications or alerts
    - Details generated by the system/tools (Focus on what the USER explicitly shares about themselves).
    ...
    try:
        ...
        logger.info(f"📋 Extracted {len(extraction.facts)} candidate fact(s) from dialogue.")
